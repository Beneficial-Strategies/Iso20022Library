...
public class AccountNotification10ValidatorTests
{
    private readonly AccountNotification10Validator _sut = new();

    private static Choices.Party12Choice.Agent ValidDebtor() =>
        new()
        {
            FinancialInstitutionIdentification = new FinancialInstitutionIdentification8(),
        };

    private static BranchAndFinancialInstitutionIdentification5 ValidAgent() =>
        new() { FinancialInstitutionIdentification = new FinancialInstitutionIdentification8() };

    private static CashAccount24 ValidAccount(string id = "ACC-001") =>
        new()
        {
            Identification = new Choices.AccountIdentification4Choice.Other { Identification = id },
        };

    private static NotificationItem5 MinimalItem(string currency = "USD", decimal amount = 100m) =>
        new()
        {
    ...
    private static AccountNotification10 ValidMessage() =>
        new()
        {
