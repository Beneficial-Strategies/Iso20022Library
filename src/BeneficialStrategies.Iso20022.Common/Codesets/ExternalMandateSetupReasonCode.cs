// Copyright 2026 Jeff Ward, Beneficial Strategies. Usage subject to license of enclosing library.

using System.Diagnostics.CodeAnalysis;
using System.Runtime.Serialization;
using System.Text.Json.Serialization;
using System.Text.RegularExpressions;

namespace BeneficialStrategies.Iso20022.Codesets;

/// <summary>
/// Specifies the mandate setup reason, as published in an external mandate setup reason code set.
/// </summary>
/// <remarks>
/// The list of valid codes is an external code set published separately.
/// External code sets can be downloaded from www.iso20022.org.
/// </remarks>
[DataContract]
[Serializable]
[IsoId("_Q77v4T2MEeS6DKIV6SF6gw")]
[Description(@"Specifies the mandate setup reason, as published in an external mandate setup reason code set.|External code sets can be downloaded from www.iso20022.org.")]
[Derivations(typeof(ExternalMandateSetupReason1Code))]
[JsonConverter(typeof(Iso20022ExternalCodeJsonConverter<ExternalMandateSetupReasonCode>))]
public readonly struct ExternalMandateSetupReasonCode : IIsoExternalCode, IEquatable<ExternalMandateSetupReasonCode>
{
    /// <summary>ISO 20022 format constraint — 1 to 4 characters.</summary>
    public const string Pattern = @"^.{1,4}$";

    /// <inheritdoc/>
    public string Value { get; }

    /// <summary>Initializes a new instance with the given mandate setup reason code.</summary>
    /// <exception cref="Iso20022FormatException">Thrown when <paramref name="value"/> does not satisfy <see cref="Pattern"/>.</exception>
    public ExternalMandateSetupReasonCode(string value)
    {
        if (!Regex.IsMatch(value, Pattern))
            throw new Iso20022FormatException(typeof(ExternalMandateSetupReasonCode), value, Pattern);
        Value = value;
    }

    /// <summary>Returns <see langword="true"/> and a valid instance when <paramref name="value"/> satisfies <see cref="Pattern"/>; otherwise <see langword="false"/>.</summary>
    public static bool TryCreate(string value, [NotNullWhen(true)] out ExternalMandateSetupReasonCode result)
    {
        if (Regex.IsMatch(value, Pattern)) { result = new(value); return true; }
        result = default;
        return false;
    }

    /// <summary>Implicitly wraps a string as a <see cref="ExternalMandateSetupReasonCode"/>.</summary>
    public static implicit operator ExternalMandateSetupReasonCode(string value) => new(value);
    /// <summary>Implicitly unwraps the code to its string value.</summary>
    public static implicit operator string(ExternalMandateSetupReasonCode code) => code.Value;

    /// <inheritdoc/>
    public override string ToString() => Value ?? string.Empty;
    /// <inheritdoc/>
    public bool Equals(ExternalMandateSetupReasonCode other) => Value == other.Value;
    /// <inheritdoc/>
    public override bool Equals(object? obj) => obj is ExternalMandateSetupReasonCode other && Equals(other);
    /// <inheritdoc/>
    public override int GetHashCode() => Value?.GetHashCode() ?? 0;

    public static bool operator ==(ExternalMandateSetupReasonCode a, ExternalMandateSetupReasonCode b) => a.Equals(b);
    public static bool operator !=(ExternalMandateSetupReasonCode a, ExternalMandateSetupReasonCode b) => !a.Equals(b);
    public static bool operator ==(ExternalMandateSetupReasonCode a, string? b) => a.Value == b;
    public static bool operator !=(ExternalMandateSetupReasonCode a, string? b) => a.Value != b;
    public static bool operator ==(string? a, ExternalMandateSetupReasonCode b) => a == b.Value;
    public static bool operator !=(string? a, ExternalMandateSetupReasonCode b) => a != b.Value;
}
