...
[DataContract]
[Serializable]
[IsoId("_XZJV19p-Ed-ak6NoX_4Aeg_-1471472211")]
...
[JsonConverter(typeof(Iso20022SimpleValueJsonConverter<RestrictedFINXMax140Text>))]
public readonly struct RestrictedFINXMax140Text : IIsoSimpleValue<string>, IEquatable<RestrictedFINXMax140Text>
{
    ...
    /// </exception>
    public RestrictedFINXMax140Text(string value)
    {
        ArgumentNullException.ThrowIfNull(value);
        if (value.Length < MinLength)
            throw Iso20022FormatException.ForTooShort(typeof(RestrictedFINXMax140Text), value, MinLength);
        if (value.Length > MaxLength)
            throw Iso20022FormatException.ForTooLong(typeof(RestrictedFINXMax140Text), value, MaxLength);
        foreach (var c in value)
            if (!IsCharSetXWithNewlines(c))
                throw Iso20022FormatException.ForInvalidCharacter(typeof(RestrictedFINXMax140Text), value, AllowedCharacters);
        Value = value;
    }

    /// <summary>Returns <see langword="true"/> when <paramref name="value"/> satisfies all constraints.</summary>
    public static bool TryCreate(string? value, [NotNullWhen(true)] out RestrictedFINXMax140Text result)
    {
    ...
    /// <summary>Implicitly wraps a string as a <see cref="RestrictedFINXMax140Text"/>.</summary>
    public static implicit operator RestrictedFINXMax140Text(string value) => new(value);
    /// <summary>Implicitly unwraps to the underlying string.</summary>
    public static implicit operator string(RestrictedFINXMax140Text text) => text.Value;

    /// <inheritdoc/>
    public override string ToString() => Value ?? string.Empty;
    /// <inheritdoc/>
    public bool Equals(RestrictedFINXMax140Text other) => Value == other.Value;
    /// <inheritdoc/>
    public override bool Equals(object? obj) => obj is RestrictedFINXMax140Text other && Equals(other);
    /// <inheritdoc/>
