...
/// <summary>
/// Validates <see cref="AggregateBalanceInformation2"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
public class AggregateBalanceInformation2Validator : AbstractValidator<AggregateBalanceInformation2>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="AggregateBalanceInformation2"/>.
    /// </summary>
    public AggregateBalanceInformation2Validator()
    {
        RuleFor(x => x.HoldingValue)
            .NotEmpty()
            .WithMessage("AggregateBalanceInformation2.HoldingValue must contain at least one element (1..∞).");

        RuleFor(x => x.PriceDetails)
            .NotEmpty()
            .WithMessage("AggregateBalanceInformation2.PriceDetails must contain at least one element (1..∞).");
    }
}
