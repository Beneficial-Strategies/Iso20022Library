...
/// <summary>
/// Validates <see cref="OperationMandate5"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
/// </list>
/// </remarks>
public sealed class OperationMandate5Validator : AbstractValidator<OperationMandate5>
{
    public OperationMandate5Validator()
    {
        RuleFor(x => x.ApplicableChannel)
            .NotEmpty()
            .WithMessage("OperationMandate5.ApplicableChannel must contain at least one element (1..∞).");

        RuleFor(x => x.BankOperation)
            .NotEmpty()
            .WithMessage("OperationMandate5.BankOperation must contain at least one element (1..∞).");
    }
}
