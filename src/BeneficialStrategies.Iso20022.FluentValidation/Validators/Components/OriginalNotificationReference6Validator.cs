...
/// <summary>
/// Validates <see cref="OriginalNotificationReference6"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
/// ISO ID: _cltUHR77EeSxevWRRWxNAg
...
/// </list>
///
/// Constraints (from spec) — <b>NOT enforceable, spec/model mismatch</b>: <see cref="OriginalItem4"/>
/// (the type of <see cref="OriginalNotificationReference6.OriginalItem"/>) has only 5 real ISO
...
public class OriginalNotificationReference6Validator : AbstractValidator<OriginalNotificationReference6>
{
    public OriginalNotificationReference6Validator()
    {
        ...
        // OriginalItem: OriginalItem4, Min=1,Max=unbounded — the ValueList<T> default ([])
        // satisfies the C# type system but violates the ISO minimum. Enforce at runtime.
        RuleFor(x => x.OriginalItem)
            .NotEmpty()
            .WithMessage("OriginalNotificationReference6.OriginalItem must contain at least one element (1..∞).");
