...
/// <summary>
/// Validates <see cref="OperationMandate2"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
public class OperationMandate2Validator : AbstractValidator<OperationMandate2>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="OperationMandate2"/>.
    /// </summary>
    public OperationMandate2Validator()
    {
        RuleFor(x => x.ApplicableChannel)
            .NotEmpty()
            .WithMessage("OperationMandate2.ApplicableChannel must contain at least one element (1..∞).");

        RuleFor(x => x.BankOperation)
            .NotEmpty()
            .WithMessage("OperationMandate2.BankOperation must contain at least one element (1..∞).");
    }
}
