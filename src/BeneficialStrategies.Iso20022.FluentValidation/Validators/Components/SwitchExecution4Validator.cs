...
/// <summary>
/// Validates <see cref="SwitchExecution4"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="SwitchExecution4"/>.
    /// </summary>
    public SwitchExecution4Validator()
    {
        RuleFor(x => x.RedemptionLegDetails)
            .NotEmpty()
            .WithMessage("SwitchExecution4.RedemptionLegDetails must contain at least one element (1..∞).");

        RuleFor(x => x.SubscriptionLegDetails)
            .NotEmpty()
            .WithMessage("SwitchExecution4.SubscriptionLegDetails must contain at least one element (1..∞).");
    }
}
