...
/// <summary>
/// Validates <see cref="SwitchOrder7"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
public class SwitchOrder7Validator : AbstractValidator<SwitchOrder7>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="SwitchOrder7"/>.
    /// </summary>
    public SwitchOrder7Validator()
    {
        RuleFor(x => x.RedemptionLegDetails)
            .NotEmpty()
            .WithMessage("SwitchOrder7.RedemptionLegDetails must contain at least one element (1..∞).");

        RuleFor(x => x.SubscriptionLegDetails)
            .NotEmpty()
            .WithMessage("SwitchOrder7.SubscriptionLegDetails must contain at least one element (1..∞).");
    }
}
