...
/// <summary>
/// Validates <see cref="LoanContract4"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
/// </list>
/// </remarks>
public sealed class LoanContract4Validator : AbstractValidator<LoanContract4>
{
    public LoanContract4Validator()
    {
        RuleFor(x => x.Buyer)
            .NotEmpty()
            .WithMessage("LoanContract4.Buyer must contain at least one element (1..∞).");

        RuleFor(x => x.Seller)
            .NotEmpty()
            .WithMessage("LoanContract4.Seller must contain at least one element (1..∞).");
    }
}
