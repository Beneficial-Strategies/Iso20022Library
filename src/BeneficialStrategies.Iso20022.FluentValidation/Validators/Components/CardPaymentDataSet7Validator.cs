...
/// <summary>
/// Validates <see cref="CardPaymentDataSet7"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
public class CardPaymentDataSet7Validator : AbstractValidator<CardPaymentDataSet7>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="CardPaymentDataSet7"/>.
    /// </summary>
    public CardPaymentDataSet7Validator()
    {
        RuleFor(x => x.Transaction)
            .NotEmpty()
            .WithMessage("CardPaymentDataSet7.Transaction must contain at least one element (1..∞).");

        RuleFor(x => x.TransactionTotals)
            .NotEmpty()
            .WithMessage("CardPaymentDataSet7.TransactionTotals must contain at least one element (1..∞).");
    }
}
