...
/// <summary>
/// Validates <see cref="OriginalNotificationReference3"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
/// ISO ID: _tm3txFkyEeGeoaLUQk__nA_-1725987675
...
/// </list>
///
/// Constraints (from spec) — <b>NOT enforceable, spec/model mismatch</b>: <see cref="OriginalItemAndStatus3"/>
/// (the type of <see cref="OriginalNotificationReference3.OriginalItemAndStatus"/>) has only 5 real ISO
...
public class OriginalNotificationReference3Validator : AbstractValidator<OriginalNotificationReference3>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="OriginalNotificationReference3"/>.
    /// </summary>
    public OriginalNotificationReference3Validator()
    {
        ...
        // OriginalItemAndStatus: OriginalItemAndStatus3, Min=1,Max=unbounded — the ValueList<T> default ([])
        // satisfies the C# type system but violates the ISO minimum. Enforce at runtime.
        RuleFor(x => x.OriginalItemAndStatus)
            .NotEmpty()
            .WithMessage("OriginalNotificationReference3.OriginalItemAndStatus must contain at least one element (1..∞).");
