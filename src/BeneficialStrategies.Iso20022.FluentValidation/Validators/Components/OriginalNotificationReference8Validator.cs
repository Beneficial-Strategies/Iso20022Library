...
/// <summary>
/// Validates <see cref="OriginalNotificationReference8"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
/// ISO ID: _2R6OTTq4EeWZFYSPlduMhw
...
/// </list>
///
/// Constraints (from spec) — <b>NOT enforceable, spec/model mismatch</b>: <see cref="OriginalItem5"/>
/// (the type of <see cref="OriginalNotificationReference8.OriginalItem"/>) has only 5 real ISO
...
public class OriginalNotificationReference8Validator : AbstractValidator<OriginalNotificationReference8>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="OriginalNotificationReference8"/>.
    /// </summary>
    public OriginalNotificationReference8Validator()
    {
        ...
        // OriginalItem: OriginalItem5, Min=1,Max=unbounded — the ValueList<T> default ([])
        // satisfies the C# type system but violates the ISO minimum. Enforce at runtime.
        RuleFor(x => x.OriginalItem)
            .NotEmpty()
            .WithMessage("OriginalNotificationReference8.OriginalItem must contain at least one element (1..∞).");
