...
/// <summary>
/// Validates <see cref="LoanContract3"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
/// </list>
/// </remarks>
public class LoanContract3Validator : AbstractValidator<LoanContract3>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="LoanContract3"/>.
    /// </summary>
    public LoanContract3Validator()
    {
        RuleFor(x => x.Buyer)
            .NotEmpty()
            .WithMessage("LoanContract3.Buyer must contain at least one element (1..∞).");

        RuleFor(x => x.Seller)
            .NotEmpty()
            .WithMessage("LoanContract3.Seller must contain at least one element (1..∞).");
    }
}
