...
/// <summary>
/// Validates <see cref="OriginalNotificationReference2"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
/// ISO ID: _Xwl_UGgpEd-V-sxUfn6-AA
...
/// </list>
///
/// Constraints (from spec) — <b>NOT enforceable, spec/model mismatch</b>: <see cref="OriginalItemAndStatus2"/>
/// (the type of <see cref="OriginalNotificationReference2.OriginalItemAndStatus"/>) has only 5 real ISO
...
public class OriginalNotificationReference2Validator : AbstractValidator<OriginalNotificationReference2>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="OriginalNotificationReference2"/>.
    /// </summary>
    public OriginalNotificationReference2Validator()
    {
        ...
        // OriginalItemAndStatus: OriginalItemAndStatus2, Min=1,Max=unbounded — the ValueList<T> default ([])
        // satisfies the C# type system but violates the ISO minimum. Enforce at runtime.
        RuleFor(x => x.OriginalItemAndStatus)
            .NotEmpty()
            .WithMessage("OriginalNotificationReference2.OriginalItemAndStatus must contain at least one element (1..∞).");

        // Account, RelatedAccount (CashAccount16, 0..1), AccountServicer, DebtorAgent,
