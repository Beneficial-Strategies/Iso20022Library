...
/// <summary>
/// Validates <see cref="CardPaymentDataSet40"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
/// </list>
/// </remarks>
public sealed class CardPaymentDataSet40Validator : AbstractValidator<CardPaymentDataSet40>
{
    public CardPaymentDataSet40Validator()
    {
        RuleFor(x => x.Transaction)
            .NotEmpty()
            .WithMessage("CardPaymentDataSet40.Transaction must contain at least one element (1..∞).");

        RuleFor(x => x.TransactionTotals)
            .NotEmpty()
            .WithMessage("CardPaymentDataSet40.TransactionTotals must contain at least one element (1..∞).");
    }
}
