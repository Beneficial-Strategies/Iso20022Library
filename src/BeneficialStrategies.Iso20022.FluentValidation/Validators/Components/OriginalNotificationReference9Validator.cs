...
/// <summary>
/// Validates <see cref="OriginalNotificationReference9"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
/// ISO ID: _GYfxD249EeiU9cctagi5ow
...
/// </list>
///
/// Constraints (from spec) — <b>NOT enforceable, spec/model mismatch</b>: <see cref="OriginalItemAndStatus6"/>
/// (the type of <see cref="OriginalNotificationReference9.OriginalItemAndStatus"/>) has only 5 real ISO
...
/// </list>
/// </remarks>
public class OriginalNotificationReference9Validator : AbstractValidator<OriginalNotificationReference9>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="OriginalNotificationReference9"/>.
    /// </summary>
    public OriginalNotificationReference9Validator()
    {
        ...
        // OriginalItemAndStatus: OriginalItemAndStatus6, Min=1,Max=unbounded — the ValueList<T> default ([])
        // satisfies the C# type system but violates the ISO minimum. Enforce at runtime.
        RuleFor(x => x.OriginalItemAndStatus)
            .NotEmpty()
            .WithMessage("OriginalNotificationReference9.OriginalItemAndStatus must contain at least one element (1..∞).");

        // Account, RelatedAccount (CashAccount38, 0..1), AccountServicer, DebtorAgent,
        // IntermediaryAgent (BranchAndFinancialInstitutionIdentification6, 0..1), AccountOwner,
        // Debtor (Party40Choice_, 0..1), TotalAmount (ActiveOrHistoricCurrencyAndAmount, 0..1):
