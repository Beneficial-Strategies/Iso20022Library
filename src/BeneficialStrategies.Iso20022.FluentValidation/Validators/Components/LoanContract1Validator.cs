...
/// <summary>
/// Validates <see cref="LoanContract1"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
/// </list>
/// </remarks>
public sealed class LoanContract1Validator : AbstractValidator<LoanContract1>
{
    public LoanContract1Validator()
    {
        RuleFor(x => x.Buyer)
            .NotEmpty()
            .WithMessage("LoanContract1.Buyer must contain at least one element (1..∞).");

        RuleFor(x => x.Seller)
            .NotEmpty()
            .WithMessage("LoanContract1.Seller must contain at least one element (1..∞).");
    }
}
