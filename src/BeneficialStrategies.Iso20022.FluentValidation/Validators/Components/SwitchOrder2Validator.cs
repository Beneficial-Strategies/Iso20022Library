...
/// <summary>
/// Validates <see cref="SwitchOrder2"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
public class SwitchOrder2Validator : AbstractValidator<SwitchOrder2>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="SwitchOrder2"/>.
    /// </summary>
    public SwitchOrder2Validator()
    {
        RuleFor(x => x.RedemptionLegDetails)
            .NotEmpty()
            .WithMessage("SwitchOrder2.RedemptionLegDetails must contain at least one element (1..∞).");

        RuleFor(x => x.SubscriptionLegDetails)
            .NotEmpty()
            .WithMessage("SwitchOrder2.SubscriptionLegDetails must contain at least one element (1..∞).");
    }
}
