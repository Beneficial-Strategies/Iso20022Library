...
/// <summary>
/// Validates <see cref="SwitchOrder3"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
public class SwitchOrder3Validator : AbstractValidator<SwitchOrder3>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="SwitchOrder3"/>.
    /// </summary>
    public SwitchOrder3Validator()
    {
        RuleFor(x => x.RedemptionLegDetails)
            .NotEmpty()
            .WithMessage("SwitchOrder3.RedemptionLegDetails must contain at least one element (1..∞).");

        RuleFor(x => x.SubscriptionLegDetails)
            .NotEmpty()
            .WithMessage("SwitchOrder3.SubscriptionLegDetails must contain at least one element (1..∞).");
    }
}
