...
/// <summary>
/// Validates <see cref="StatementGroup4"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
/// ISO ID: _cI-GsdcZEeqRFcf2R4bPBw
...
/// </remarks>
public class StatementGroup4Validator : AbstractValidator<StatementGroup4>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="StatementGroup4"/>.
    /// </summary>
    public StatementGroup4Validator()
    {
        ...
        RuleFor(x => x.BillingStatement)
            .NotEmpty()
            .WithMessage("StatementGroup4.BillingStatement must contain at least one element (1..*).");

        RuleFor(x => x.SenderIndividualContact)
            .Must(list => list.Count <= 2)
            .WithMessage("StatementGroup4.SenderIndividualContact must not contain more than 2 element(s) (0..2).");

        RuleFor(x => x.ReceiverIndividualContact)
            .Must(list => list.Count <= 2)
            .WithMessage("StatementGroup4.ReceiverIndividualContact must not contain more than 2 element(s) (0..2).");
    }
}
