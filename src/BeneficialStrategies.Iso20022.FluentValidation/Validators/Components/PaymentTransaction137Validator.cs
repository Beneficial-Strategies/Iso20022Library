...
public sealed class PaymentTransaction137Validator : AbstractValidator<PaymentTransaction137>
{
    ...
    private static readonly Regex UetrRegex = new(
        @"^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$",
    ...
    public PaymentTransaction137Validator()
    {
        ...
        RuleFor(x => x.OriginalUETR)
            .Matches(UetrRegex)
                .WithMessage("PaymentTransaction137.OriginalUETR must be a valid UUID v4 identifier " +
        ...
        When(x => x.OriginalInterbankSettlementAmount is not null, () =>
        {
            RuleFor(x => x.OriginalInterbankSettlementAmount!.Currency)
                .NotEmpty()
                .Matches(CurrencyRegex)
                    .WithMessage("PaymentTransaction137.OriginalInterbankSettlementAmount.Currency " +
                                 "must be a valid ISO 4217 three-letter currency code (e.g. EUR, USD).");

            RuleFor(x => x.OriginalInterbankSettlementAmount!.Amount)
                .GreaterThan(0)
                    .WithMessage("PaymentTransaction137.OriginalInterbankSettlementAmount.Amount must be greater than zero.");
        });
