...
/// <summary>
/// Validates <see cref="CreditTransferTransaction82"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
/// ISO ID: _eXCdsbpXEfCPZvweyV2nEg
...
///   <item><term>PreviousInstructingAgent1/2/3, InstructingAgent, InstructedAgent, IntermediaryAgent1/2/3, DebtorAgent, CreditorAgent</term><description>BranchAndFinancialInstitutionIdentification8 — optional (0..1) each</description></item>
///   <item><term>PreviousInstructingAgent1/2/3Account, IntermediaryAgent1/2/3Account, DebtorAccount, DebtorAgentAccount, CreditorAgentAccount, CreditorAccount</term><description>CashAccount40 — optional (0..1) each</description></item>
///   <item><term>UltimateDebtor/UltimateCreditor</term><description>BranchAndFinancialInstitutionIdentification8 — optional (0..1) each</description></item>
...
/// </list>
///
/// <c>InterbankSettlementDate</c>, <c>SettlementPriority</c>, and <c>ExpiryDateTime</c> are
...
/// tying its fields to <c>OriginalTransactionReference45</c>'s siblings, if any, are enforced
/// higher up the graph.
///
/// <c>InterbankSettlementAmount</c> (required, <c>ActiveCurrencyAndAmount</c>, plain record, not
...
/// validator across all 13 agent fields, one <c>CashAccount40</c> validator across all 9 account
/// fields — see the two constructors below.
/// </remarks>
public class CreditTransferTransaction82Validator : AbstractValidator<CreditTransferTransaction82>
{
    ...
    /// </summary>
    public CreditTransferTransaction82Validator(
    ...
    )
    {
        ...
        RuleFor(x => x.InterbankSettlementAmount.Currency)
            ...
            .WithMessage(
                "CreditTransferTransaction82.InterbankSettlementAmount.Currency must be a valid ISO 4217 three-letter currency code (e.g. EUR, USD)."
            );
        RuleFor(x => x.InterbankSettlementAmount.Amount)
            .GreaterThan(0)
            .WithMessage("CreditTransferTransaction82.InterbankSettlementAmount.Amount must be greater than zero.");
        ...
        When(x => x.UltimateDebtor is not null, () => RuleFor(x => x.UltimateDebtor).SetValidator(agentValidator!));
        ...
        When(x => x.UltimateCreditor is not null, () => RuleFor(x => x.UltimateCreditor).SetValidator(agentValidator!));
