...
/// <summary>
/// Validates <see cref="ChargesPerTransaction3"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
/// ISO ID: _4DOhsTEyEe6g-ffJsqGiSA
...
/// </remarks>
public class ChargesPerTransaction3Validator : AbstractValidator<ChargesPerTransaction3>
{
    /// <summary>
    ...
    /// Validator for each item of the <c>Record</c> collection (ChargesPerTransactionRecord3, 1..∞).
    /// </param>
    public ChargesPerTransaction3Validator(IValidator<ChargesPerTransactionRecord3> recordValidator)
    {
        ...
        // ChargesAccountAgentAccount (CashAccount40, 0..1), TotalChargesPerTransaction
        ...
        // Record: ChargesPerTransactionRecord3, 1..∞ — NotEmpty on the collection, plus its own
        // validator applied per item.
        RuleFor(x => x.Record)
            .NotEmpty()
            .WithMessage("ChargesPerTransaction3.Record must contain at least one element (1..*).");
    ...
    /// validated by its own default validator (<see cref="ChargesPerTransactionRecord3Validator"/>).
    /// Convenience constructor for callers not using a DI container.
    /// </summary>
    public ChargesPerTransaction3Validator()
        : this(new ChargesPerTransactionRecord3Validator()) { }
}
