...
/// <summary>
/// Validates <see cref="TradeContract4"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
/// </list>
/// </remarks>
public class TradeContract4Validator : AbstractValidator<TradeContract4>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="TradeContract4"/>.
    /// </summary>
    public TradeContract4Validator()
    {
        RuleFor(x => x.Buyer)
            .NotEmpty()
            .WithMessage("TradeContract4.Buyer must contain at least one element (1..∞).");

        RuleFor(x => x.Seller)
            .NotEmpty()
            .WithMessage("TradeContract4.Seller must contain at least one element (1..∞).");
    }
}
