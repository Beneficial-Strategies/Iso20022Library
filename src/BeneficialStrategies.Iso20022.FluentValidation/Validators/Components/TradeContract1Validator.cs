...
/// <summary>
/// Validates <see cref="TradeContract1"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
public class TradeContract1Validator : AbstractValidator<TradeContract1>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="TradeContract1"/>.
    /// </summary>
    public TradeContract1Validator()
    {
        RuleFor(x => x.Buyer)
            .NotEmpty()
            .WithMessage("TradeContract1.Buyer must contain at least one element (1..∞).");

        RuleFor(x => x.Seller)
            .NotEmpty()
            .WithMessage("TradeContract1.Seller must contain at least one element (1..∞).");
    }
}
