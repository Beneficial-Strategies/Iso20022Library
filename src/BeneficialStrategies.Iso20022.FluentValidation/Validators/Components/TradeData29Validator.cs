...
/// <summary>
/// Validates <see cref="TradeData29"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
/// </list>
/// </remarks>
public class TradeData29Validator : AbstractValidator<TradeData29>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="TradeData29"/>.
    /// </summary>
    public TradeData29Validator()
    {
        RuleFor(x => x.ReportStatistics)
            .NotEmpty()
            .WithMessage("TradeData29.ReportStatistics must contain at least one element (1..∞).");

        RuleFor(x => x.TransactionStatistics)
            .NotEmpty()
            .WithMessage("TradeData29.TransactionStatistics must contain at least one element (1..∞).");
    }
}
