...
/// <summary>
/// Validates <see cref="AggregateBalanceInformation3"/> per the ISO 20022 specification.
/// </summary>
/// <remarks>
...
public class AggregateBalanceInformation3Validator : AbstractValidator<AggregateBalanceInformation3>
{
    /// <summary>
    /// Initializes a new instance of the validator for <see cref="AggregateBalanceInformation3"/>.
    /// </summary>
    public AggregateBalanceInformation3Validator()
    {
        RuleFor(x => x.HoldingValue)
            .NotEmpty()
            .WithMessage("AggregateBalanceInformation3.HoldingValue must contain at least one element (1..∞).");

        RuleFor(x => x.PriceDetails)
            .NotEmpty()
            .WithMessage("AggregateBalanceInformation3.PriceDetails must contain at least one element (1..∞).");
    }
}
