...
/// <summary>
/// Validates <see cref="LiquidityDebitTransferV07"/> (camt.051.001.07) per the ISO 20022
/// specification.
/// </summary>
/// <remarks>
/// ISO ID: _R3PRqzEZEe6kQ-WGAhcVPQ
...
///
/// Scope
/// The LiquidityDebitTransfer message is sent by a member to the transaction administrator. It
...
///   <item><term>LiquidityDebitTransfer</term><description>LiquidityDebitTransfer4 — required (1..1)</description></item>
...
///
/// Dependency injection: <c>MessageHeader</c>, <c>LiquidityDebitTransfer</c>, and the
/// <c>SupplementaryData</c> collection are each validated by an injected
/// <see cref="IValidator{T}"/> rather than a hardcoded <c>new</c> — see the two constructors
/// below.
/// </remarks>
public class LiquidityDebitTransferV07Validator : AbstractValidator<LiquidityDebitTransferV07>
{
    /// <summary>
    /// Initializes a new instance using dependency injection: the caller supplies the validator
    /// for the required <c>MessageHeader</c> and <c>LiquidityDebitTransfer</c> building blocks
    ...
    /// </param>
    /// <param name="liquidityDebitTransferValidator">
    /// Validator for the required <c>LiquidityDebitTransfer</c> building block
    /// (LiquidityDebitTransfer4, 1..1).
    /// </param>
    ...
    /// </param>
    public LiquidityDebitTransferV07Validator(
        IValidator<MessageHeader1> messageHeaderValidator,
        IValidator<LiquidityDebitTransfer4> liquidityDebitTransferValidator,
        IValidator<SupplementaryData1> supplementaryDataValidator
    )
    {
        RuleFor(x => x.MessageHeader).SetValidator(messageHeaderValidator);
        RuleFor(x => x.LiquidityDebitTransfer).SetValidator(liquidityDebitTransferValidator);
    ...
    /// <c>LiquidityDebitTransfer</c>, and the <c>SupplementaryData</c> collection are each
    /// validated by their own default validator (<see cref="MessageHeader1Validator"/>,
    /// <see cref="LiquidityDebitTransfer4Validator"/>, <see cref="SupplementaryData1Validator"/>).
    /// Convenience constructor for callers not using a DI container.
    /// </summary>
    public LiquidityDebitTransferV07Validator()
        : this(
            new MessageHeader1Validator(),
            new LiquidityDebitTransfer4Validator(),
